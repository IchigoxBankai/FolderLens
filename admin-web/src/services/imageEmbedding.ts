...
class ImageEmbeddingService {
  ...
  public async initialize(onProgress?: ModelProgressCallback): Promise<{ processor: any; model: any }> {
    ...
    this.initPromise = (async () => {
        ...
        try {
          if (onProgress) {
            onProgress({
              status: 'loading_model',
              progress: 25,
              message: `Loading visual AI model (${device.toUpperCase()})...`
            });
          }
            ...
            progress_callback: (progressInfo: any) => {
              if (onProgress && progressInfo.status === 'progress') {
                ...
                onProgress({
                  ...
                  message: `Downloading visual AI: ${progressInfo.file || ''} (${percent}%)`
                });
              }
            }
          });
      ...
      if (!loadedProcessor || !loadedModel) {
        throw new Error('Failed to initialize CLIP visual embedding model on both WebGPU and WASM.');
      }
  ...
  /**
   * Generates a genuine 512-dimensional normalized CLIP visual feature embedding vector.
   */
  public async generateEmbedding(
    ...
    if (onProgress) {
      onProgress({
        ...
        message: 'Extracting 512-dim CLIP visual embedding...'
      });
    }
